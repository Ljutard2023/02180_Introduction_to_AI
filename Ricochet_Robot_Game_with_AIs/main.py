...
---------------
  Opens with F5 or the "Run All AIs" button.
    BFS, DFS, A*1, A*2, and A*3 run sequentially in a background worker.
...
class AIWindow(tk.Toplevel):
    """
    Secondary window that runs all selected solvers (BFS, DFS, A*1, A*2,
    A*3) sequentially in a background worker and
    displays:
    ...
    def _run_all_solvers(self) -> None:
        """
        Run all selected solvers sequentially in one daemon worker thread.
        """
        threading.Thread(target=self._run_all_solvers_seq, daemon=True).start()

    def _run_all_solvers_seq(self) -> None:
        """Execute each solver one-by-one (no parallel solver execution)."""
        for name, cls in self._solvers.items():
            self._run_one(name, cls)
