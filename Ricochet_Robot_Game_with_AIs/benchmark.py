...
Runs all AI solvers on a set of randomly generated board states and reports:
  ...
  • Average time (seconds) per solver
    • Average visited nodes per solver
...
class BenchmarkWindow(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        headers = ['Solver', 'Scenarios', 'Avg Moves', 'Avg Time (s)',
               'Avg Visited', 'Found']
        widths  = [8,        10,          10,           13,
               11,            6]
        ...
        # Bar chart canvas
        self._chart_cv = tk.Canvas(self, width=700, height=80,
    ...
    def _run_benchmark(self) -> None:
        ...
        # Accumulate solved metrics: name → list of (moves, time)
        results: dict[str, list[tuple[int, float]]] = {n: [] for n in BENCH_SOLVERS}
        # Accumulate effort metrics on all runs: name → list of visited nodes
        visited_counts: dict[str, list[int]] = {n: [] for n in BENCH_SOLVERS}
        ...
        for scenario_idx in range(NUM_SCENARIOS):
            ...
            for name, cls in BENCH_SOLVERS.items():
                ...
                visited_counts[name].append(int(getattr(solver, 'last_visited_nodes', 0)))
        ...
        self.after(0, self._show_results, results, visited_counts)
    ...
    def _show_results(self, results: dict[str, list[tuple[int, float]]],
                      visited_counts: dict[str, list[int]]) -> None:
        self._running = False
        ...
        for name, data in results.items():
            ...
            n    = NUM_SCENARIOS
            found = len(data)
            vc_data = visited_counts[name]
            if data:
                avg_m = sum(m for m, _ in data) / len(data)
                avg_t = sum(t for _, t in data) / len(data)
            else:
                avg_m = avg_t = 0.0
            avg_v = (sum(vc_data) / len(vc_data)) if vc_data else 0.0
            ...
            lbls[3].config(text=f"{avg_v:.0f}" if vc_data else "—")
            lbls[4].config(text=str(found))
